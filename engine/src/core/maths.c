...
f32 jsinf(f32 x) { return sinf(x); }
f32 jcosf(f32 x) { return cosf(x); }
f32 jtanf(f32 x) { return tanf(x); }
f32 jacosf(f32 x) { return acosf(x); }
f32 jsqrtf(f32 x) { return sqrtf(x); }
f32 jabsf(f32 x) { return fabsf(x); }
...
f32 frandom(void) { return (f32)random() / (f32)RAND_MAX; }
...
/* ============================== VECTOR 2 ================================== */
vec2 vec2_create(f32 x, f32 y) { return (vec2){.comp1.x = x, .comp1.y = y}; }
...
f32 vec2_length_square(vec2 v) {
    return v.comp1.x * v.comp1.x + v.comp1.y * v.comp1.y;
}

f32 vec2_length(vec2 v) { return sqrtf(vec2_length_square(v)); }
...
vec3 vec3_multi_scalar(vec3 v, f32 s) {
    return (vec3){.comp1.x = v.comp1.x * s,
                  .comp1.y = v.comp1.y * s,
                  .comp1.z = v.comp1.z * s};
}

f32 vec3_length_square(vec3 v) {
    return v.comp1.x * v.comp1.x + v.comp1.y * v.comp1.y +
           v.comp1.z * v.comp1.z;
}

f32 vec3_length(vec3 v) { return sqrtf(vec3_length_square(v)); }
...
b8 vec3_compared(vec3 v1, vec3 v2, f32 tolerance) {
    if (jabsf(v1.comp1.x - v2.comp1.x) > tolerance) {
        return false;
    }
    if (jabsf(v1.comp1.y - v2.comp1.y) > tolerance) {
        return false;
    }
    if (jabsf(v1.comp1.z - v2.comp1.z) > tolerance) {
        return false;
    }
    return true;
}
...
f32 vec4_length_square(vec4 v) {
    return v.comp1.x * v.comp1.x + v.comp1.y * v.comp1.y +
           v.comp1.z * v.comp1.z + v.comp1.w * v.comp1.w;
}

f32 vec4_length(vec4 v) { return sqrtf(vec4_length_square(v)); }
...
mat4 mat4_euler_x(f32 angle_rad) {
    mat4 result = mat4_identity();
    f32 cos = cosf(angle_rad);
    f32 sin = sinf(angle_rad);
...
mat4 mat4_euler_y(f32 angle_rad) {
    mat4 result = mat4_identity();
    f32 cos = cosf(angle_rad);
    f32 sin = sinf(angle_rad);
...
mat4 mat4_euler_z(f32 angle_rad) {
    mat4 result = mat4_identity();
    f32 cos = cosf(angle_rad);
    f32 sin = sinf(angle_rad);
...
mat4 mat4_row_ortho(f32 left, f32 right, f32 bottom, f32 top, f32 near,
                    f32 far) {
    mat4 result;
    memset(result.data, 0, sizeof(f32) * 16);
...
mat4 mat4_column_ortho(f32 left, f32 right, f32 bottom, f32 top, f32 near,
                       f32 far) {
    mat4 result;
    memset(result.data, 0, sizeof(f32) * 16);
...
mat4 mat4_row_perspective(f32 fov_rad, f32 aspect_ratio, f32 near, f32 far) {
    f32 half_tan = tanf(fov_rad * 0.5f);
    mat4 result;
    memset(result.data, 0, sizeof(f32) * 16);
...
mat4 mat4_column_perspective(f32 fov_rad, f32 aspect_ratio, f32 near, f32 far) {
    f32 half_tan = tanf(fov_rad * 0.5f);
    mat4 result;
...
mat4 mat4_euler_xyz(f32 x_rad, f32 y_rad, f32 z_rad) {
...
mat4 quat_to_rotation_matrix(quat q, vec3 center) {
    ...
    f32 xx = q.comp1.x * q.comp1.x;
    f32 yy = q.comp1.y * q.comp1.y;
    f32 zz = q.comp1.z * q.comp1.z;
    f32 xy = q.comp1.x * q.comp1.y;
    f32 xz = q.comp1.x * q.comp1.z;
    f32 yz = q.comp1.y * q.comp1.z;
    f32 wx = q.comp1.w * q.comp1.x;
    f32 wy = q.comp1.w * q.comp1.y;
    f32 wz = q.comp1.w * q.comp1.z;
